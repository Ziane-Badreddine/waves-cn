...
| `onDiscard` | `() => void` | — | Fires when the user discards the take. |
| `onError` | `(error: Error) => void` | — | Fires on microphone / MediaRecorder errors. |
| `maxDuration` | `number` | — | Auto-stop after N seconds. |
| `mimeType` | `string` | browser default | e.g. `"audio/webm;codecs=opus"`. |
| `audioBitsPerSecond` | `number` | `128000` | Bitrate passed to `MediaRecorder`. |
| `deviceId` | `string` | — | Specific microphone device id. |
| `disabled` | `boolean` | `false` | Disables all controls. |
| `showWaveform` | `boolean` | `true` | Show the live waveform while recording. |
| `showTimer` | `boolean` | `true` | Show the elapsed-time counter. |
| `waveColor` | `string` | `var(--primary)` | Live waveform color. |
| `progressColor` | `string` | `var(--background)` | Progress color of the live waveform. |
...
| `barHeight` | `number` | — | Bar height multiplier passed to wavesurfer. |
...
The microphone stream and the record plugin are released when the component unmounts, including mid-recording.
