...
```bash
npx shadcn@latest add audio-timeline
```
...
Wrap in a `"use client"` component when passing function props (onPlay, onFinish, onTimeUpdate):
...
Passed to `topTimeline` or `bottomTimeline` to control how the ruler renders.
...
| `primaryLabelInterval` | `number` | `5` | `1` | Seconds between labeled (primary) ticks. |
...
**Theme reactivity** — Colors are in the `useEffect` dependency array via `useCssVar`. Toggling dark mode will correctly destroy and recreate WaveSurfer with the updated palette. The brief re-render shows the loading spinner.

**CORS** — WaveSurfer fetches audio from the browser. Ensure your audio URL is same-origin or served with `Access-Control-Allow-Origin`.

**Server Components** — Same rule as `AudioPlayer`: render directly from a Server Component with no function props, or wrap in `"use client"` if you need callbacks.

**Timeline at low zoom** — At very low `minPxPerSec` values (e.g. `1–5`), timeline ticks may overlap. Tune `timeInterval` and `primaryLabelInterval` to match your expected zoom range.

**Prop reactivity** — Only `src` and the resolved CSS color values trigger reinitialization. Props like `barWidth`, `waveHeight`, and `defaultZoom` are read once at mount. To make them reactive, add them to the `useEffect` dependency array and call `ws.setOptions()` accordingly.