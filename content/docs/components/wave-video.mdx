...
| ---- | ---- | ------- | ----------- |
| `src` | `string` | — | **(Required)** Video URL. Changing it recreates the wavesurfer instance. |
| `url` | `string` | — | **Deprecated.** Alias of `src`, kept for v1 compatibility. |
| `title` | `string` | — | Track title displayed above the waveform. |
| `showVideo` | `boolean` | `true` | Render the native `<video>` element. |
...
| `videoProps` | `VideoHTMLAttributes` | — | Extra props forwarded to `<video>` (`poster`, `loop`, `muted`, …). `src`, `ref`, `controls` and `playsInline` are managed internally. |
