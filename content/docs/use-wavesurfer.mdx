...
Only `url`, `media`, `plugins`, `peaks`, `duration`, `sampleRate`, `backend` and `mediaControls` recreate the instance. Every other option (`height`, `barWidth`, colors, `audioRate`, …) is applied in place through `wavesurfer.setOptions()` and never re-decodes the audio.

`plugins` and `peaks` are compared by reference, so keep them in `useMemo`.

## Imperative calls

Playback state (speed, volume, position) is still best driven imperatively:
...
If you need play/pause/seek/volume state with matching actions, use [`useWavePlayer`](/docs/use-wave-player) together with `WavesurferPlayer` instead.
...
| `plugins` | `GenericPlugin[]` | `[]` | Memoized plugin templates (cloned on each create). |
| `peaks` | `Array<Float32Array \| number[]>` | — | Precomputed peaks, memoized. |
| `duration` | `number` | — | Duration in seconds, used with `peaks`. |
...
| `isPlaying` | `boolean` | `true` while wave is playing. |
