...
const WavesurferPlayer = memo((props: WavesurferProps): ReactElement => {
  ...
  for (const key in rest) {
    if (isEventProp(key)) eventProps[key as keyof OnWavesurferEvents] = rest[key as keyof typeof rest] as never;
  ...
  const progressColor = (options.progressColor as string | undefined) ?? WAVESURFER_DEFAULTS.progressColor;
  ...
  const minPxPerSec = (options.minPxPerSec as number | undefined) ?? WAVESURFER_DEFAULTS.minPxPerSec;
  const cursorWidth = (options.cursorWidth as number | undefined) ?? WAVESURFER_DEFAULTS.cursorWidth;
  ...
  useEffect(() => {
    ...
    const unsubs = eventEntries.map((name) => {
      ...
      return ws.on(event, (...args) =>
        (eventsRef.current[name as keyof OnWavesurferEvents] as WavesurferEventHandler<WaveSurferEvents[typeof event]>)?.(
          ws, ...args,
        ),
      );
    });
  ...
  useEffect(() => {
    wsRef.current?.setOptions({ waveColor: resolvedWaveColor, progressColor: resolvedProgressColor });
  ...
  return (
    <div className={className} style={{ position: "relative" }}>
      {!isReady && (
        <div style={{
          height, width: "100%", position: "absolute", inset: 0,
  ...
  const STRUCTURAL_KEYS = ["url", "height", "barWidth", "barGap", "barRadius",
    "minPxPerSec", "cursorWidth", "dragToSeek", "waveColor", "progressColor"];
  return STRUCTURAL_KEYS.every((k) => prev[k as keyof WavesurferProps] === next[k as keyof WavesurferProps]);
});
...
export function useWavesurfer({
  ...
  useEffect(() => {
    ...
    setWavesurfer(ws);
    const unsubs = [
      ws.on("load",       () => { setIsReady(false); setIsPlaying(false); setCurrentTime(0); }),
      ws.on("ready",      () => { setIsReady(true); }),
      ws.on("play",       () => { setIsPlaying(true); }),
      ws.on("pause",      () => { setIsPlaying(false); }),
      ws.on("timeupdate", () => { setCurrentTime(ws.getCurrentTime()); }),
      ws.on("destroy",    () => { setIsReady(false); setIsPlaying(false); }),
    ];
    return () => { unsubs.forEach((fn) => fn()); ws.destroy(); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [url, height, barWidth, barGap, barRadius, minPxPerSec]);

  useEffect(() => {
    wavesurfer?.setOptions({ waveColor: resolvedWaveColor, progressColor: resolvedProgressColor });
